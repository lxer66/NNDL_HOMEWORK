...
def test_visualization():
    ...
    # 模拟训练历史
    train_losses = [1.0 - i * 0.015 + np.random.normal(0, 0.05) for i in range(epochs)]
    val_losses = [1.0 - i * 0.012 + np.random.normal(0, 0.08) for i in range(epochs)]
    train_accuracies = [0.1 + i * 0.017 + np.random.normal(0, 0.03) for i in range(epochs)]
    val_accuracies = [0.1 + i * 0.015 + np.random.normal(0, 0.04) for i in range(epochs)]
    
    # 确保准确率在合理范围内
    train_accuracies = [max(0.1, min(0.99, acc)) for acc in train_accuracies]
    val_accuracies = [max(0.1, min(0.99, acc)) for acc in val_accuracies]
    ...
    for i in range(num_classes):
        confusion_matrix[i, i] += 50
    
    class_names = [str(i) for i in range(num_classes)]
    plot_confusion_matrix(confusion_matrix, class_names)
    
    # 创建模拟类别准确率
    class_accuracies = {str(i): 0.85 + np.random.normal(0, 0.1) for i in range(num_classes)}
    for i in range(num_classes):
        class_accuracies[str(i)] = max(0.7, min(0.99, class_accuracies[str(i)]))
